...
const add1ToCart = (cart) => {
    var myBtn = document.querySelectorAll('[dataadd-id]')
    for (let j = 0; j < myBtn.length; j++) {
        myBtn[j].addEventListener('click', function() {
...
const displayCartTable = (cart) => {  // Affiche le contenu du panier dans un tableau
    for (let i = 0; i < cart.length ; i++) {
        ...
        var total = cart[i].price * cart[i].quantity   
                ...
                '<td><button dataadd-id= "' + cart[i].id + '" data-color= "' + cart[i].color + '" class = "btn-add">+1</button><button data-id= "' + cart[i].id + '" data-color= "' + cart[i].color + '" class = "btn-delete">-1</button></td>' +
                '<td>'+ total + '€</td>' +
